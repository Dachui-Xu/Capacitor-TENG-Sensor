...
uint8_t FDC2214_GetChannelData(FDC2214_channel_t channel, uint32_t *data)
{
    ...
    switch(channel)
    {
		...
		status = FDC2214_Read16(FDC2214_Addr, FDC2214_STATUS); // 读取状态寄存器

    while(timeout && !(status & bitUnreadConv))//检查是否有未读数据
    {
			//	HAL_Delay(10); // 增加延迟，避免频繁读取
 ...
 *  @since      v1.0
 *  @else       rcount 是 RCOUNT 的缩写，代表 "Repeat Count"，即重复计数。
							它是 FDC2214 芯片中用于控制每次电容测量转换的采样计数值。
							具体来说，RCOUNT 决定了 FDC2214 在每个测量周期中用于计数的时间长度，进而影响转换精度。
 *  Sample usage:       FDC2214_SetRcount(FDC2214_Channel_0, 0x0FFF);
 */
void FDC2214_SetRcount(FDC2214_channel_t channel, uint16_t rcount)
{
    if(rcount <= 0x00FF) return; //不能小于254
 ...
 *  @param      count       CHx_SETTLECOUNT
							较高精度应用：SETTLECOUNT 建议设置为 500 - 2000。
							速度优先应用：SETTLECOUNT 建议设置为 100 - 500。
 *  @since      v1.0
...
double FDC2214_CalculateCapacitance(double frequency, float inductance, float capacitance)
{
    ...
    cap = (double)(1.0 / (inductance * (2 * PI * frequency) * (2 * PI * frequency)))*1e12 - capacitance - 4; // fdc2214本身的寄生电容为4pf
...
uint8_t FDC2214_Init(void)
{
    ...
    if(deviceID == FDC2214_ID)
    {
        ...
        FDC2214_SetSettleCount(FDC2214_Channel_0, 2000);//200 cycles 等待时间
        FDC2214_SetSettleCount(FDC2214_Channel_1, 2000);//200 cycles 等待时间
        FDC2214_SetSettleCount(FDC2214_Channel_2, 2000);//200 cycles 等待时间
        FDC2214_SetSettleCount(FDC2214_Channel_3, 2000);//200 cycles 等待时间
        ...
        FDC2214_SetINTB(0);//关闭INTB中断
			/*
				...
				FDC2214_SetActiveChannel(FDC2214_Channel_1);//开启通道1
				FDC2214_SetActiveChannel(FDC2214_Channel_2);//开启通道2
