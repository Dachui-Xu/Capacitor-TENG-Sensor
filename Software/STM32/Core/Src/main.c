...
extern uint32_t ch0,ch1,ch2,ch3;
...
void printf_DMA(const char *format,...)
{
    ...
    length = vsnprintf((char*)_dbg_Buff, sizeof(_dbg_Buff), (char*)format, args); // 保证不超出缓冲区
    va_end(args);

    // 先检查DMA是否空闲，防止上一次传输未完成时再次启动
